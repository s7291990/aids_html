...
$(function () {
    ...
    $("body").on("click", ".period-tab", function () {
        $(this).parents(".period-tabs").eq(0).find(".period-tab").removeClass("active");
        $(this).addClass("active");
    });
    // 탭메뉴 (기본)
    ...
    $("body").on("click", ".btn-date", function () {
        $(this).parents(".date-tab-container").eq(0).find(".btn-date").removeClass("active");
        $(this).addClass("active");
    });
    // 라디오 선택 (직접입력 선택시)
    ...
    calendar: function () {
        ...
        var baseOptions = {
            ...
            beforeShow: function (input, inst) {
                setTimeout(function () { decorateYearSelect(inst); }, 0);
            },
